...
class PCF8591(ADCDevice):
    ...
    def analogRead(self, chn): # PCF8591 has 4 ADC input pins, chn:0,1,2,3
        high = self.bus.read_byte_data(self.address, self.cmd+chn)
        low = self.bus.read_byte_data(self.address, self.cmd+chn + 1)
        value = (high << 8) + low
        print(value)
        return value
        ...
        #return self.bus.read_block_data(self.address, self.cmd+chn)
